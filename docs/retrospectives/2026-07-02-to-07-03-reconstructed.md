...
(`93db73c`: "restructure plan prompt rules into four labelled sections").

The `depends_on` field (a list of file paths a step depends on) went through the same failure four
times in one day: the model encoded it as a bare string instead of a real list (`b9f5321`, fixed by
tolerating the string), then as a quoted string-array (`86a9b62`, fixed by tolerating that shape
too), and that very fix then over-matched and broke a *different*, already-working case — bracket
type annotations elsewhere in the same YAML (`a8a55dc`). A parallel field in a different prompt
(dependency proposals) needed the identical class of fix a day later: list-item indentation broken
(`a184854`), then a degenerate empty-bracket value causing an outright panic (`12e1eaf`).

By the end of the period, prompt formatting itself had become a deliberate, systematized fix
target — not just wording, but visual structure: `228f418` and `1df5531` rolled out consistent
`##`/`###` section headers across every skill and plan prompt, and `38eb415` physically relocated a
...
Neither failure class was fully solved by a prompt-only fix. `0cabf3d` shows this most directly:
...
governed — moving the identical rule closer to the point of generation (`38eb415`) is presented as
...
*(Reconstructed inference, closely paraphrasing `0cabf3d`'s own commit message, not invented.)*
