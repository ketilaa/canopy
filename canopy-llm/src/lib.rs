...
Required project setup (must exist before tests run):
  ...
  tsconfig.json with esModuleInterop: true (generated by npx tsc --init)
  jest.config.js in service root (generated by npx ts-jest config:init)

Framework stack:
  jest      — test runner and mocking (jest.fn(), jest.mock())
  supertest — HTTP integration: request(app).post('/api/...')

Import paths — tests live in tests/ (one level below service root):
  import request from 'supertest'          // default import — esModuleInterop required
  import app from '../src/app'             // ../src/... NOT ../../src/...
  import { Foo } from '../src/models/Foo'  // relative imports ONLY — no @aliases

NEVER use TypeScript path aliases (@services/..., @src/..., etc.).
NEVER write require() — ES import syntax only.
...
Service unit test pattern (mock dependencies inline with as any):
  ...
  mockRepo.save.mockResolvedValue({ id: 'uuid', ...payload })  // always pass a value
  mockRepo.findById.mockResolvedValue(null)                    // null is a valid value

Forbidden — fix immediately if found:
  import { app } from '...'          → import app from '../src/app' (default export, correct path)
  import ... from '@services/...'   → use relative path ../src/...
  import ... from '../../src/...'   → wrong depth from tests/; use ../src/...
  require(...)                       → use ES import
  mockResolvedValue()                → mockResolvedValue always requires an argument;
                                       use mockResolvedValue(undefined) for void returns";
