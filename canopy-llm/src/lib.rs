...
### Service unit test example
  ...
  beforeEach(() => {
    mockRepo = { saveProduct: jest.fn(), getProductById: jest.fn() } as any
  ...
  it('creates a product and publishes an event', async () => {
    const saved = { id: 'test-id-123', createdAt: new Date(), modifiedAt: null, name: 'Widget',
                    manufacturer: 'Acme', model: 'X1', categoryIds: ['cat1'], description: '' }
    mockRepo.saveProduct.mockResolvedValue(saved)
    mockPublisher.connect.mockResolvedValue(undefined)
    mockPublisher.publish.mockResolvedValue(undefined)
    mockPublisher.disconnect.mockResolvedValue(undefined)
    const result = await service.createProduct({ name: 'Widget', manufacturer: 'Acme',
                                                 model: 'X1', categoryIds: ['cat1'] })
